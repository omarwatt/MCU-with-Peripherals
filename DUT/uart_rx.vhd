...
entity UART_RX is
    Port (
        ...
        PENA_i      : in  std_logic; -- ADDED: UTCL parity enable
        PEV_i       : in  std_logic; -- ADDED: UTCL parity type
...
begin
    ...
    DOUT <= rx_data;
    RX_BUSY_o   <= '0' when rx_pstate = idle else '1';
    -- -------------------------------------------------------------------------
        ...
        if (rising_edge(CLK)) then
            if (RST = '1') then
                ...
                if (rx_clk_en = '1' AND rx_output_reg_en = '1') then
                    ...
                    if PENA_i = '1' then
                        PARITY_ERROR <= rx_parity_error; -- make the error seperate from dout_vld
                    else
                        PARITY_ERROR <= '0';
                    end if;
                else
